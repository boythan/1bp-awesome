...
let lastLocation
const runKalmanOnLocations = (rawData, kalmanConstant = 1000) =>
  rawData
    .map((location) => ({
      ...location,
...
const getStaticImageMap = (latlng, apiKey) => {
  return `https://maps.googleapis.com/maps/api/staticmap?center=${latlng}&zoom=17&markers=color:pink%7Clabel:o%7Csize=100*100%7C${latlng}&size=1000x1000&key=${apiKey}`
}

export default {
  runKalmanOnLocations,
